...
`0.1.10 → 0.1.11` 的 Setup 真实更新链路已完整通过。portable lane 已验证公开 Release、固定基线、目标下载 SHA-256、三个页面的全局提示、60 秒就绪保持和用户确认后的旧进程退出；随后 15 分钟内没有新健康租约或目标进程，诊断为 `portableHelperCode=missing`。这次结果否定了“复制 helper 到稳定目录即可闭环”的既有假设。`0.1.11` portable 仍可从公开 Release 直接运行；自动替换与启动尚未通过验收，下一次只在有新产品修复与新诊断证据时继续。
...
| Windows portable | 后台下载同架构新版 portable 并核对 size 与 SHA-256；完成后所有页面右下角显示“退出并运行新版”。当前自动启动未通过 N-1 验收，用户可从 Release 手动运行新版 portable | `-portable.exe`、`SHA256SUMS.txt` |
...
- Windows: Setup SHA-256 `8f77166b5057354d523b0150a32470cb083a3e2c4086d6cc879014bb3950cdfa`；portable SHA-256 `ae044b6a825b9bfd6c5e597a449900f3a59d163aa0b018102cc1db2b882da133`；两包均未签名
- N-1 acceptance: 首次运行 `29678501162` 在产品启动前发现验收脚本遗漏 `0.1.10` 固定基线；提交 `cd821d8d04da7dead16aa6685f9e57d0e50625e6` 补齐并通过测试。最终运行 `29678656926` 中 Setup `0.1.10 → 0.1.11` 完整通过；portable 完成下载、校验、三页提示、60 秒就绪保持和旧进程退出，随后未启动新版，终态为 `leaseUpdatedAfterConsent=False`、`targetProcessCount=0`、`portableHelperCode=missing`。保留 Setup 通过结论，portable 用户当前走 Release 手动启动，自动链路等待下一次正常产品修复
- Security review: 本地包未包含数据库、环境文件、日志、torrent 或 qBit profile，个人路径文本 `0` 命中；公开附件名为精确 `13` 项，服务端 SHA-256 与校验清单一致
...
- N-1 acceptance: Setup `0.1.8 → 0.1.9` 在运行 `29515559280` 完整通过；portable 验收定位到 NSIS wrapper 清理竞态，修复随 `0.1.10` 发布。后续 `0.1.10 → 0.1.11` 证明该修复仍未自动启动新版，完整终态见 `v0.1.11 Release Record`
