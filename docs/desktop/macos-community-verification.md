...
4. 用 H.264/AAC MP4 验证 Safari 播放、拖动 Range、刷新恢复进度、倍速、全屏、截图、`.vtt` / `.srt` 字幕。
5. 用 Safari 不支持的 MKV 或编码验证清晰的格式反馈与系统播放器入口；不能声称所有容器都能在 iOS Safari 播放。
6. 更改下载目录，再添加第二个测试任务；新任务进入新目录，旧文件保持原位。
...
7. 在 Mac 移除这台设备，手机最迟 30 秒后失去受保护页面与 API 访问。
8. 再次配对，然后关闭局域网访问；全部设备会被清空，手机会话最迟 30 秒后失效，Next 恢复只监听 `127.0.0.1`。
9. 连续输入八次错误配对码，原配对码必须失效。
