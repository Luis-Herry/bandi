...
- **季度番表信息更完整**：Bangumi 提供主体资料，长门番堂补充播出时间、开播日期、总话数、正版入口、官网与 PV。
...
季度番剧库支持一月、四月、七月、十月四个档期，也能按年份、类型、题材、地区、来源和评分筛选。Bangumi 与长门番堂的高置信同作会合并到一个条目；候选存在歧义时停止自动关联，降低串番风险。
...
桌面主进程维护独立的 qBittorrent profile。正常使用时，界面只呈现“服务是否可用、速度、剩余空间和任务状态”，端口、账号与 Web UI 都藏在后台。
...
| 文件 | 适合谁 | 使用方式 |
|---|---|---|
| `追番中心-Setup-*-x64.exe` | 日常长期使用 | 可选择安装目录，并创建桌面与开始菜单快捷方式 |
| `追番中心-*-x64-portable.exe` | 临时体验或移动硬盘 | 直接运行；首次自解压可能需要等待一会儿 |
...
| 番剧库 | 按季度浏览与筛选，查看 Bangumi + 长门番堂资料，加入想看 |
...
| 来源 | 用途 | 边界 |
|---|---|---|
| [Bangumi](https://bangumi.tv/) | 动漫主体资料、集数、人物、评分 | 主要动画资料来源 |
| [长门番堂](https://yuc.wiki/) | 季度档期、播出时间、话数、正版入口、官网、PV | 补充来源；页面保留来源链接与 CC BY-NC-SA 4.0 署名 |
...
| Anime Garden / 用户 RSS | 单集资源发现 | 用户自行选择来源并确认合法性 |
| qBittorrent | 本地下载执行 | 使用独立受管 profile |
